...
- Name: homepage.spec.ts >> Homepage >> pricing section shows both plans
- Location: tests\e2e\homepage.spec.ts:12:7
...
Locator: locator('text=Individual')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=Individual')
...
```ts
  ...
  4  |   test("loads and shows key elements", async ({ page }) => {
  5  |     await page.goto("/");
  6  |     await expect(page).toHaveTitle(/AESDR/);
  7  |     await expect(page.locator("text=Stop Surviving")).toBeVisible();
  8  |     await expect(page.locator("text=GET ACCESS")).toBeVisible();
  9  |     await expect(page.locator("text=SIGN IN")).toBeVisible();
  10 |   });
  11 | 
  12 |   test("pricing section shows both plans", async ({ page }) => {
  13 |     await page.goto("/#pricing");
> 14 |     await expect(page.locator("text=Individual")).toBeVisible();
     |                                                   ^ Error: expect(locator).toBeVisible() failed
  15 |     await expect(page.locator("text=Team")).toBeVisible();
  16 |   });
  17 | 
  18 |   test("sign in link navigates to login", async ({ page }) => {
  19 |     await page.goto("/");
  20 |     await page.click("text=SIGN IN");
