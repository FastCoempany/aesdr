...
/**
 * HoleRasterLayer — Option A. Raster sprite rabbit holes.
 *
 * Each "hole" position from the Underland scatter is rendered by
 * placing a procedurally-generated PNG sprite (/public/holes/hole-N.png)
 * via SVG <image>. The sprites carry their own lighting, depth gradient,
 * cast shadow, and dirt clod texture — none of that is drawn by the
 * Layer at runtime.
 *
 * The 4 sprite variants differ in sun angle and pile orientation, so
 * placing them at varied rotations/scales produces a non-mechanical
 * scatter.
 *
 * The rest of the Underland scaffolding (surface line, strata, tunnels,
 * chambers, roots, hare tracks, hazard tags) is kept identical to the
 * other variants — only the "stones" rendering swaps to <image>.
 */
...
const SPRITE_PATHS = ["/holes/hole-1.png", "/holes/hole-2.png", "/holes/hole-3.png", "/holes/hole-4.png"];

export function HoleRasterLayer() {
  ...
  const layout = useMemo(() => {
    ...
    // Hole positions — raster sprites scattered across the page
    type HoleSprite = { cx: number; cy: number; size: number; sprite: number; rot: number };
    const holes: HoleSprite[] = [];
    for (let i = 0; i < 18; i++) {
      const sz = 110 + rng() * 90;
      holes.push({
        cx: 80 + rng() * 1280,
        ...
        size: sz,
        sprite: Math.floor(rng() * SPRITE_PATHS.length),
        rot: (rng() - 0.5) * 60,
      });
    }
  ...
  return (
    <>
      <IntensityToggle label="Hole · Raster" value={intensity} onChange={setIntensity} />
      ...
      {/* L2: ink scaffolding (strata lines, tunnels, chambers, roots, tracks) */}
      <svg
        aria-hidden="true"
      ...
      >
        ...
        <text x="40" y={SURFACE_Y - 14} fontFamily="'Space Mono', monospace" fontSize="9" letterSpacing="0.24em" fill="#6B6B6B">
          — SURFACE
        </text>
        ...
        <g opacity="0.55">
          {layout.strata.map((b, i) => (
            <g key={i}>
              ...
              <text x="40" y={b.y - 6} fontFamily="'Space Mono', monospace" fontSize="8" letterSpacing="0.22em" fill="#1A1A1A" opacity="0.75">
                — {b.label}
              </text>
            </g>
          ))}
        ...
        <g transform="translate(72 124)" opacity="0.88">
          ...
          <text x="0" y="20" fontFamily="'Space Mono', monospace" fontSize="9" letterSpacing="0.26em" fill="#6B6B6B">
            OPTION A · PNG SPRITES · DIRT EXCAVATED
          </text>
          <line x1="0" y1="30" x2="380" y2="30" stroke="#1A1A1A" strokeWidth="0.6" opacity="0.55" />
        </g>
      </svg>

      {/* L3: RASTER hole sprites — placed on TOP so the dirt sits over the chambers/tracks */}
      <svg
        aria-hidden="true"
        viewBox={`0 0 1440 ${pageH}`}
        preserveAspectRatio="xMidYMin slice"
