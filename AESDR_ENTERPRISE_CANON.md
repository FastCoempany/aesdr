# AESDR / Enterprise — Canon + Build Spec
...
`/enterprise` is the B2B / channel-partner face of AESDR. It exists to give sales-org buyers (managers, enablement, RevOps, L&D, fractional services, LMS platforms) a surface they can engage with on their terms — without diluting or replacing the consumer-direct survival voice that aesdr.com is built on.
...
- **Not a generic "behavioral OS" pivot of the whole company.** The behavioral-OS framing lives only in `/enterprise/*`. Consumer surfaces stay in the survival voice ("for the AE/SDR grinding through 80 dials and a 9% reply rate").
...
| Surface | Register |
|---|---|
| `/enterprise` landing | Manager-confident editorial. Playfair headlines. Survival framing acknowledged but framed as *the reason this works for orgs* ("AEs and SDRs treat it as a survival guide, which is why they finish it") |
| `/enterprise/channel` | Channel-partnership ops voice. Less prose, more table. Pitch-mode. |
| `/enterprise/pricing` | Direct, transactional. Numbers and what they include. |
...
**Primary:** `AESDR / Enterprise`

Read as "AESDR for the enterprise side." The slash separator signals subsidiary relationship without using corporate suffixes like "Inc" or "Pro."

**Short form (in URL, in headers when space-constrained):** `AESDR / Enterprise`
...
The wordmark on subsidiary pages uses the iris-shimmer gradient (same `var(--iris)` + 4s shimmer keyframe used on consumer brand wordmark and role chips), but with the `/ Enterprise` suffix in `var(--mono)` (Barlow Condensed) at a smaller size and `var(--muted)` color.

Spec:
```
[AESDR]      <-- Playfair Display, 900 italic, iris-shimmer fill
[ / Enterprise]   <-- Barlow Condensed, 700, .15em letter-spacing,
...
Three React components under `app/enterprise/_components/SubLogo.tsx`:
...
| Role | Consumer brand | Subsidiary `/enterprise` |
|---|---|---|
...
The single most important visual decision: **iris-shimmer is the primary accent on `/enterprise/*`.** This is the instant family-recognition signal — same gradient as the consumer wordmark, but used aggressively instead of rationed. Buttons, key CTAs, KPI numbers, divider lines, hover states, focus rings.
...
| `--hand` | `'Caveat', cursive` | **Do not use on `/enterprise/*`.** Caveat is consumer-brand DNA (Michael's voice / margin annotations). Subsidiary is structural; handwriting reads wrong here. |
...
- **Comparison table.** "Build it yourself / Existing LMS course / AESDR Enterprise" — structured side-by-side, not narrative.
...
Reusable components to build (in `app/enterprise/_components/`):
...
| `<ChannelCategoryBlock>` | Channel-partner pitch block. Title + 4-column micro-table. |
...
| `<EnterpriseFooter>` | Subsidiary footer. Includes "Powered by aesdr.com" line + back link + contact. |
...
Leponeus is consumer-brand DNA. On `/enterprise/*`:
...
B2B audiences see mascots and unconsciously categorize the product as consumer / playful / not-for-enterprise. We won't fight that. The mascot stays beloved on aesdr.com and almost invisible on `/enterprise/*`.
...
app/enterprise/
  ...
  enterprise.module.css            Subsidiary-scoped styles
  _components/
    EnterpriseNav.tsx
    EnterpriseFooter.tsx
    KPIStat.tsx
    BuyerModeCard.tsx
    ChannelCategoryBlock.tsx
    ComparisonTable.tsx
    SpecSection.tsx
    InlineCTA.tsx
  partners/
    page.tsx                  /enterprise/channel — channel pitch
  pricing/
  ...
  curriculum/
    page.tsx                  /enterprise/curriculum
  diagnostic/
    page.tsx                  /enterprise/diagnostic
  implementation/
    page.tsx                  /enterprise/implementation — manager guide
  integrations/
    page.tsx                  /enterprise/integrations

app/api/enterprise-inquiry/
  route.ts                    POST handler for /enterprise/contact form
                              Emails hello@aesdr.com with [/enterprise inquiry] prefix
```
...
|---|---|
| `/enterprise` | Convince a sales manager / enablement leader / RevOps that AESDR is the behavioral foundation their first-year AEs and SDRs are missing. CTA: book a walkthrough. |
...
Each block is a `<ChannelCategoryBlock>` with:
- Category name as h2
...
- Primary CTA: `Partnership inquiry` → `/enterprise/contact?source=channel`

### 5.3 `/enterprise/pricing`
...
2. **Custom Enterprise — Contact us** (featured / center tier)
   ...
   - CTA: `Talk to us` → `/enterprise/contact?source=pricing-custom`
...
3. **White-label / Reseller — Contact us**
   ...
   - CTA: `Talk to us` → `/enterprise/contact?source=pricing-wl`
...
**Backend:** POST to `/api/enterprise-inquiry`. Server action validates, rate-limits (5/hr per IP), sanitizes, sends formatted email via `lib/email.ts` to `hello@aesdr.com` with subject `[/enterprise inquiry] {role} from {company}`. Body includes all form fields + source + timestamp + IP.
...
|---|---|---|---|
| **Subsidiary visual identity** | CSS module + component library | `app/enterprise/enterprise.module.css` + `app/enterprise/_components/` | Foundation for everything |
| **Landing page** | `/enterprise` route | `app/enterprise/page.tsx` | All sections per §5.1 |
| **Partners page** | `/enterprise/channel` route | `app/enterprise/channel/page.tsx` | Five category blocks per §5.2 |
...
implemented as **print-friendly web pages** under `/enterprise/downloads/*` with
...
Detailed pitch angles for each category, used both for the `/enterprise/channel` page content and for outbound conversations.
...
Honest list. Public on `/enterprise/integrations`. Updated as items move from roadmap to live.
...
We don't build any of these in the initial sprint. They live on `/enterprise/integrations` as honest roadmap. Building begins when an actual partner conversation generates the pull.
...
- **Routing:** Native App Router. `app/enterprise/` with nested `layout.tsx` that scopes the subsidiary visual identity.
- **Styling:** CSS Modules (matches existing pattern). One `enterprise.module.css` for shared subsidiary styles + per-component module files if components grow.
...
- **Rate limiting:** Reuse `lib/rate-limit.ts`. Apply to `/api/enterprise-inquiry` at 5/hr per IP.
- **Analytics:** None on `/enterprise/*` initially. Matches consumer brand restraint. Add if a real ops need emerges.
...
app/api/enterprise-inquiry/route.ts
...
`/enterprise/*` stays behind the `COMING_SOON` gate. No changes needed to `proxy.ts` — the existing logic redirects all non-admin, non-bypass-cookie traffic to `/coming-soon`, and `/enterprise/*` falls under that.

Admins (`antaeus.coe@gmail.com`, `mrcoe7@gmail.com`) and anyone holding the `aesdr_cs_bypass` cookie access `/enterprise/*` normally. This is the desired behavior for the pre-launch period — work in private, share preview links via the bypass URL pattern when needed.

### 10.4 AdminChip integration

`components/AdminChip.tsx`'s `QUICK_LINKS` constant gets the `/enterprise/*` routes added **beneath a visual divider line**, so the admin dropdown reads as two sections: existing AESDR navigation, then a separated cluster of subsidiary routes.
...
const QUICK_LINKS: ({ label: string; href: string; note?: string } | { divider: true })[] = [
  ...
  { label: "Home", href: "/" },
  { divider: true },
  { label: "/enterprise · Landing", href: "/enterprise", note: "Enterprise subsidiary" },
  { label: "/enterprise · Partners", href: "/enterprise/channel" },
  { label: "/enterprise · Pricing", href: "/enterprise/pricing" },
  { label: "/enterprise · Contact", href: "/enterprise/contact" },
  { label: "/enterprise · Curriculum", href: "/enterprise/curriculum" },
  { label: "/enterprise · Diagnostic", href: "/enterprise/diagnostic" },
  { label: "/enterprise · Implementation", href: "/enterprise/implementation" },
...
`/api/enterprise-inquiry` posts emit one email to `hello@aesdr.com`. Subject format:

```
[/enterprise inquiry] {role} from {company} ({team-size})
```

Example: `[/enterprise inquiry] Sales enablement from Acme Corp (25–49 AEs and SDRs)`

Body format (HTML, plain-text fallback):

```
A new /enterprise inquiry came in.
...
```jsx
<a href="/enterprise" style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--muted)" }}>
  for sales orgs →
</a>
```
...
**Scope:**
- `app/enterprise/layout.tsx` (subsidiary nav, footer, accent scope)
- `app/enterprise/enterprise.module.css` (shared subsidiary styles)
- `app/enterprise/_components/` (all eight reusable components)
- `app/enterprise/page.tsx` (landing — full content per §5.1)
...
**Scope:**
- `app/enterprise/channel/page.tsx`
- `app/enterprise/pricing/page.tsx`
- `app/enterprise/contact/page.tsx` + `ContactForm.tsx`
- `app/api/enterprise-inquiry/route.ts`
- New `sendTeamsInquiry()` helper in `lib/email.ts`

**Why second:** The four most action-oriented pages. After this PR, `/enterprise` is a working B2B surface that can capture inquiries.

### PR 3 — Spec pages (~5–7 days)

**Scope:**
- `app/enterprise/curriculum/page.tsx` (with all 12 learning-outcome blocks written)
- `app/enterprise/diagnostic/page.tsx` (with 6–8 dimensions specified)
- `app/enterprise/implementation/page.tsx` (full manager guide content)
- `app/enterprise/integrations/page.tsx`
...
**Scope:**
- `public/enterprise/aesdr-manager-implementation-guide.pdf`
- `public/enterprise/aesdr-completion-certificate-template.pdf`
- `public/enterprise/aesdr-partner-one-pager.pdf`
- `public/enterprise/aesdr-diagnostic-instrument.pdf`
...
**Scope:**
- `components/AdminChip.tsx` — add divider + `/enterprise/*` quick links (§10.4)
- `app/page.tsx` — add "for sales orgs →" footer link (§10.6)
- Visual fidelity pass across all `/enterprise/*` pages
...
- Affiliate tracking on `/enterprise/*` *(separate concern — affiliate program is Bilal's engagement, not this build)*
...
| 2 | 2026-05-18 | Use `/enterprise` (plural) as the route | `/team` (singular) already exists as a member team-dashboard route. `/enterprise` is free and reads naturally as "for sales teams." |
| 3 | 2026-05-18 | Sub-brand name: `AESDR / for Teams` | Matches the value-prop angle (behavioral operating system); slash separator signals subsidiary; "for Teams" felt more distinctive than "Teams" or "Enterprise" at the time. |
| 3a | 2026-05-19 | Sub-brand renamed to `AESDR / Enterprise`; URL moved from `/teams/*` to `/enterprise/*`; channel-partnerships page moved from `/teams/partners` to `/enterprise/channel` | Founder direction: keep "Partners" reserved for the consumer-side affiliate hub at `/partners`; the B2B subsidiary needs a separate word. "Enterprise" matches the audience (companies buying for their salespeople) and removes the `/teams/*` extra path segment. |
...
| 6 | 2026-05-18 | Mascot demoted to footer monogram on subsidiary | B2B audiences read mascots as consumer / playful. Mascot stays beloved on aesdr.com; near-invisible on `/enterprise/*`. |
...
| 8 | 2026-05-18 | Single small "for sales orgs →" footer link from main site to `/enterprise` | Per Antaeus: "auxiliary detached building." Minimal bridge; subsidiary is found via outbound, not consumer-funnel cross-promo. |
| 9 | 2026-05-18 | Add `/enterprise/*` routes to AdminChip dropdown BENEATH a divider line | Per Antaeus: keep admin nav clean; separated cluster for subsidiary. |
