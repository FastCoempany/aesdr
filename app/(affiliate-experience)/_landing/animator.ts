/**
 * Imperative animation engine for ExperienceLanding (affiliate-prospect fork).
 *
 * Forked from components/landing-sequence/animator.ts. Differences from the
 * production engine:
 *   - No skip affordance: the skip button, the Escape-to-skip key handler, and
 *     the onSkip option are all removed. The prospect cannot bail out early.
 *   - No fast-forward: clicking / Space / Enter no longer forces the current
 *     typing step to complete. Every line types at its natural per-character
 *     pace; scenes still advance automatically on schedule.
 *   - No analytics: drops the `@/lib/analytics` track() calls (the prospect
 *     flow does its own tracking elsewhere).
 *   - New callbacks: onStarted (once, at boot), onRolePicked (on fork pick),
 *     onReachedEnd (once, when the final CTA zone first becomes visible).
 *   - hideAccessCta: when true the final "Get Access" CTA overlay is never
 *     rendered visible; onReachedEnd still fires at the same scroll point.
 *
 * Phases:
...
export function runAnimator(refs: AnimatorRefs, opts: AnimatorOptions): () => void {
  ...
  let reachedEndFired = false;

  const timers = new Set<ReturnType<typeof setTimeout>>();
  const listeners: Array<() => void> = [];

  function fireReachedEnd() {
   ...
   *
   * Fork change: there is no fast-forward. The old `forced` short-circuit
   * (set by a global click / Space / Enter handler) is gone, so every line
   * types at its natural per-character pace and cannot be skipped.
   */
  ...
  function runTerminal(r: Role, onDone: () => void) {
    ...
    schedule(() => {
      ...
      function nextLine() {
        ...
        let ci = 0;
        // Fork change: no fast-forward — terminal lines always type at their
        // natural pace and cannot be skipped.
        function typeChar() {
          if (paused) return;
          if (ci >= stripped.length) {
            body.textContent = stripped;
            i++;
            schedule(nextLine, 260);
            return;
          }
          ci++;
  ...
  function attachZoomScroll() {
    ...
    function render(s: LayerStates) {
      // End-zone callback: fire once the prospect first reaches the final CTA
      // zone (or scrolls past the entire zoom section). This is the same
      // ctaVisibleZone / pastZoom signal the production CTA uses, so it fires
      // even when hideAccessCta suppresses the visible overlay below.
      if (s.ctaVisibleZone || s.pastZoom) fireReachedEnd();
      ...
      // CTA — suppressed entirely when hideAccessCta is set (this flow ends
      // elsewhere, not at checkout). The onReachedEnd wiring above still runs.
      if (refs.cta && !opts.hideAccessCta) {
        if (s.ctaVisibleZone) {
  ...
  /* ─────────── Boot ───────────
   *
   * Fork change: no global click/keydown advance handler and no skip-button
   * listener. The prospect must sit through the typing in full — there is no
   * fast-forward and no skip.
   */
  ...
  // animation is provably progressing.
