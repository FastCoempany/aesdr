...
} from "@/lib/affiliate-kit-tokens";
import { readRequestMeta } from "@/lib/req-meta";

export const runtime = "nodejs";

const PRIVATE_BASE = "/affiliates/kit-private";
...
export async function GET(request: Request) {
  ...
  if (!token) {
    return NextResponse.redirect(new URL("/affiliates/kit-private?e=missing", url.origin));
  }
  ...
  if (!verified.ok) {
    ...
    return NextResponse.redirect(
      new URL(`/affiliates/kit-private?e=${verified.reason}`, url.origin),
    );
  }
  ...
  if (!resolved) {
    ...
    return NextResponse.redirect(
      new URL("/affiliates/kit-private?e=revoked", url.origin),
    );
  }
