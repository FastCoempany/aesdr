...
**Deliverable:** Build specification for AESDR's internal reporting dashboard. The single surface where the founder + ops review pilot performance, affiliate-attribution, and the funnel from "page view" to "net commission paid." One dashboard, reviewable in under 15 minutes, runs every business day.
**Audience:** AESDR-internal — founder, ops. The dashboard is the source-of-truth for the metrics surfaced (in aggregate, affiliate-safe) on D25 weekly reports.
...
> **Placeholder convention:** No affiliate placeholders. Internal spec.
...
| **Quarterly cohort** | Which affiliate archetypes are converting; which are worth a second pilot? | Weekly, manual review | Founder |
...
For each currently-running pilot. One panel per pilot. Multiple panels render side-by-side if multiple pilots run in parallel (rare per canon §1.3 — a handful of affiliates, not a marketplace).

### Per-pilot panel structure

**Header:**
- Pilot ID + Affiliate name + Pilot dates (start → end)
...
**Affiliate-side metrics (from D25 affiliate-share):**
- Send 1: open rate, click rate, replies-of-quality
- Send 2: same
- Cross-check vs registrations attributed to the affiliate-slug
...
|---|---|---|
| Revenue by affiliate archetype over time | Quarterly buckets | Identifies which archetype (community / coach / creator / alumni / hybrid) is producing returns |
...
| ICP-fit assessment trend | Categorical (tight/acceptable/off — from D33 §6) | Are we getting better at vetting affiliates? |
...
| Founder-time distribution | Per-week pie chart | Validation: founder time on affiliate conversations / workshops / Admissions / canon-revision / other |
...
| Affiliate-share (D25) | Open rate, click rate, replies | Manual; weekly per D25 |
| L&D brief send log | Per-prospect L&D-brief sends + outcomes | Manual; ops updates per send |
| D27 scorecards | Pre-pilot baseline | Manual; updated on affiliate status change |
...
The hand-stitched version is intentional — per canon §1.3, building a heavyweight dashboard before there are 3+ active pilots is over-engineering. v1 ships at the cost of operator time, which is the right scale for a small affiliate program.
...
- A **simple internal Next.js dashboard** at `dashboard.aesdr.com` (server-side calls to Stripe + AESDR DB + manual-input layer for affiliate-share metrics). Owned by AESDR; auditable; no third-party dependency.
...
- **A affiliate-facing surface.** Affiliates see aggregated metrics in D25 weekly reports, not raw operator data.
...
- **Public.** The dashboard is internal. Affiliate aggregates flow to D25; market aggregates flow to D38 launch announcements. The dashboard itself is auditable but not published.
...
- **Whether to expose any portion of View 3 to the affiliate.** Default: **no.** Cohort data identifies other affiliates by archetype, even if not by name. Per canon §12 confidentiality and the §10.4 D22-inheritance non-exclusivity language, cross-affiliate data stays internal.
