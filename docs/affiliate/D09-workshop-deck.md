...
**Deliverable:** Slide-by-slide spec for the live 60-minute AESDR pilot workshop. Single canonical deck, affiliate-co-brandable via the lockup zone (canon §6.6). Same deck for every affiliate pilot — only the affiliate-intro slot, the replay-link slot, and the lockup change.
**Audience:** Live workshop attendees (workshop-first pilot motion, canon §1.1 + §7.1). Audience composition skews early-career SDR / AE per the affiliate ICP matrix in `affiliate-seeding-deep-research-report.md` §ICP.
...
|---|---|---|
| Affiliate intro | ≤ 2 min | Affiliate ad-libs, AESDR-supplied script (slide 02) |
...
### Slide 02 — Affiliate intro slot (AFFILIATE AD-LIB, AESDR-SUPPLIED SCRIPT)

**Layout:** Affiliate co-brand lockup centered, cream background, `--display` italic 36pt: *Welcome — [PARTNER_NAME] hosting AESDR.* Below it, mono `--cond`: `2 minutes max`.

**Affiliate script (supplied; affiliate ad-libs from this — never reads verbatim):**
...
**Speaker notes for affiliate:** Cap at 2 minutes. Do not pitch the offer. Do not discuss pricing. Do not ad-lib about AESDR's content beyond the supplied script. Hand off cleanly. Per canon §7.1, affiliate intro is ≤ 2 minutes by contract.
...
**Content:** Lifts verbatim from `docs/affiliate/D06-workshop-cta-offer.md`. Do not rewrite here. Refresh the placeholder values per pilot:
- Workshop slug `[WORKSHOP_SLUG]`
- Affiliate slug `[PARTNER_SLUG]`
...
| 02 Affiliate intro | Affiliate ad-lib from supplied script (≤ 2 min) |
...
- Is the deck delivered to affiliates as a PDF preview (no speaker notes) plus a `.key` / `.pptx` for record? Default: **no preview to affiliates** — affiliates see the live, like everyone else. Deck circulates only inside AESDR until/unless a specific operator earns deck-review trust (canon §16 approval gates).
