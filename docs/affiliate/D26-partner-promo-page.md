# D26 — Affiliate-Promo Landing Page

**Deliverable:** Per-affiliate co-branded landing page where Affiliate sends their audience during the promotion window. The "click here to register for the workshop" surface — the destination of every UTM-tagged link in `11-tracking-links.md` (D40 kit). Affiliate has no other AESDR-promoting page; this is it. One pilot, one page, one URL.
**Audience:** The affiliate's audience — cold to AESDR, warm to the affiliate. Most arrive having read 1–2 sentences from the affiliate ("AESDR is doing a workshop for early-career SaaS AEs and SDRs, link below"). The page does the rest of the work.
**Voice ratio:** 70 Rowan / 30 Michael per canon §3.3 (workshop registration page row, and §8.3 affiliate-promo page anatomy). Hero + outcomes in Rowan; the "who this is not for" block + the affiliate-quote slot lean Michael; affiliate-relationship disclosure in plain compliance register.
**Format:** HTML page rendered server-side at `aesdr.com/[PARTNER_SLUG]/workshop` (or equivalent). Crawlable for the affiliate-attribution window only — `noindex, nofollow` after `[PILOT_END_DATE]` per canon §1.1 (workshop-first, not evergreen).
**Canon refs:** `AFFILIATE_BRAND_CANON.md` §1.1 (workshop-first), §1.4 (borrowed trust), §3.3 (voice ratios), §6.3 (canonical layout patterns — editorial split hero), §6.4 (iris reservation), §6.6 (co-brand zone), §7.4 (attendee experience: 2-field reg form), §8.1 (registration page anatomy — full version), §8.3 (page anatomy — affiliate-promo page), §8.7 (UTM canon — hidden form fields), §10.1 (FTC), §10.5 (TCPA — separate SMS opt-in), §13 (honesty discipline).

> **Placeholder convention:** `[PARTNER_SLUG]`, `[PARTNER_NAME]`, `[PARTNER_AUDIENCE_DESCRIPTOR]` (e.g., "Apex BDR Club members," "Sales Bootcamp alumni"), `[PARTNER_QUOTE]`, `[PARTNER_QUOTE_ATTRIBUTION]`, `[WORKSHOP_TITLE]`, `[WORKSHOP_DATE]`, `[WORKSHOP_TIME_TZ]`, `[REPLAY_END_DATE_TIME_TZ]`, `[CODE_EXPIRY_DATE_TIME_TZ]`, `[PILOT_CODE]`, `[HOST_FIRST_NAME]`, `[HOST_LAST_NAME]` — filled per pilot. Affiliate-quote slot is optional and skipped if D27 didn't surface a usable quote.

---

## What's different about D26 vs D07

D07 (workshop registration page, batch 3) is AESDR's *canonical* registration page — what you build for a non-co-branded direct-traffic flow. D26 is the affiliate-co-branded variant that reuses D07's page hierarchy with three additions:

1. **Affiliate co-brand lockup** in the header band (canon §6.6).
2. **Affiliate-audience descriptor** in the hero serif body — *not* in the headline (per canon §8.3, headline stays AESDR-canonical).
3. **Affiliate-relationship disclosure** as its own block above the footer, lifting D23 Q11 verbatim (per canon §10.1 FTC material-connection rules).
4. **Optional affiliate-quote slot** in `--display` italic 36px, attributed to the affiliate operator.
...
- **Content:**
  - Workshop title, `--display` italic 700, 56pt, `--ink`:
    > *[WORKSHOP_TITLE]*
  - Affiliate-audience descriptor (the only place the affiliate audience is named, per canon §8.3), `--serif` italic 18pt, `--ink`:
...
### 6. Affiliate-quote slot (optional)
...
The quote must pass canon §10.2 approved-claims discipline. Per canon §16, affiliate quotes are pre-approved during D27.
...
- **Hidden fields (canon §8.7):** `partner_id`, `partner_type`, `cohort_id`, plus standard UTMs (`utm_source`, `utm_medium`, `utm_campaign`, `utm_content`, `utm_term`). All passed through from URL. Affiliate cannot see them; they exist for attribution.
...
### 10. Affiliate-relationship disclosure (canon §10.1, lift from D23 Q11)
...
> *AESDR runs pilot partnerships with a small number of community operators, bootcamp coaches, and creators whose audiences match the early-career SaaS-sales role we serve. You arrived through one of those affiliates' links, which means two things are true:*
>
...
>
> *— The workshop you'll attend is the same workshop every other pilot audience attends. Same deck, same offer, same pricing for this pilot. The affiliate doesn't customize the content — they bring the audience; we bring the program.*
>
...
- Affiliate-quote slot: `--display` italic 700, 36pt, `--ink`.
...
**Deliberate departures from canon:** None. The page lifts §8.3 affiliate-promo anatomy point-for-point; the affiliate-quote slot (§6 above) is optional per canon §8.3 ("optional pull-quote") and skipped when no quote is available.
...
- **FTC material-connection disclosure (§10.1):** §10 affiliate-relationship disclosure block carries the verbatim canonical disclosure (lifted from D23 Q11). **Pass.**
...
- **Forbidden claims (§10.3):** No "guaranteed promotion," no "earn $X." Affiliate quote (§6) is pre-approved during D27. **Pass.**
...
- **D23 affiliate-facing FAQ** — §9 + §10 lift Q01–Q04, Q11. **Met.**
- **D27 affiliate vetting scorecard** — affiliate-quote slot (§6) is pre-approved during D27. **Met.**
...
- **D40 master affiliate-kit folder README** — D26 is referenced as "co-branded surface AESDR ships." **Met.**
- **Co-brand lockup assets** — operationally pending per affiliate.
...
- **Per-affiliate customization budget.** Default: lockup + audience descriptor + optional quote = the only differences. Affiliates may *not* edit the headline, the outcomes, the for/not-for blocks, the FAQ, or the disclosure. Affiliates may *propose* a quote during D27; AESDR pre-approves or declines per canon §16.
- **Editorial split mobile fallback.** Default: crimson left panel stacks above cream right panel on screens <768px. Workshop title remains 44pt minimum on mobile. Affiliate-audience descriptor and date line stay in cream right.
...
- **Whether the affiliate-quote slot can hold multiple quotes** (e.g., 3 short quotes from 3 affiliate audience members). Default: **no for v1.** Multi-quote implies social-proof-stacking, which contradicts canon §13 (no fake "value stack"). Per canon §6.9, one earned quote outperforms three padded ones at thumbnail-test time.
- **Page-expiry behavior after `[PILOT_END_DATE]`.** Default: page redirects to `aesdr.com/enroll` (general program enrollment at list price). Affiliate-attribution links continue to resolve and pass attribution for the 30-day window per D22 §6.1, but the page itself stops accepting workshop registrations (the workshop already ran).
