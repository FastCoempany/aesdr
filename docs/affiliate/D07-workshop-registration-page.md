...
**Deliverable:** Full copy + structural spec for the affiliate-specific workshop registration page. One per affiliate pilot. Hosted on `aesdr.com/[affiliate-slug]/workshop` (or equivalent), with hidden attribution fields.
**Audience:** Affiliate-borrowed audience landing for the first time. ~60 seconds to convince.
**Voice ratio:** 70 Rowan / 30 Michael. Pain-led headline (D8). Honest qualification block. Single iris CTA.
**Format:** Markdown source. Renders to a Next.js page (template); content blocks are affiliate-agnostic except where bracketed.
...
│ │ affiliate co-mark    │ + outline secondary,        │   │
...
│ FAQ-LITE (4 Qs)                                        │
│ FOOTER (postal, disclosure, affiliate co-mark, year)     │
└────────────────────────────────────────────────────────┘
```
...
> `AESDR · WORKSHOP · [AFFILIATE-SLUG]`

Type: `--mono` 10pt, `.25em` letter-spacing, uppercase, color `--muted`. Position: top-left. Pairs with affiliate co-mark top-right when co-branded.
...
> `AESDR · 12 COURSES · WORKSHOP-FIRST`
...
**Affiliate co-mark** (when co-branded, per canon §6.6): `AESDR × [affiliate]` lockup, top-center of crimson panel.
...
If the affiliate is a co-host, the affiliate stub sits to the right of the host stub in the same format, with `[Affiliate name] · [Community/Program] · CO-HOST` as the mono line.
...
Optional separate consent checkbox for SMS reminder (only present when affiliate pilot uses SMS):
...
Workshop hosted by AESDR in partnership with [affiliate].
[Affiliate] receives a commission on enrollments through this page.
AESDR sets the curriculum, content, and pricing; [affiliate] vouches for fit.
