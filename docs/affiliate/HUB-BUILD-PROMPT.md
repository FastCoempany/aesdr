# AESDR Affiliate Hub — Build Prompt for Claude

## [1] TASK CONTEXT

You are Claude, working on AESDR — a 12-course sales-survival program for early-career SaaS SDRs and AEs, built on Next.js 16 / React 19 / Supabase / Stripe. The codebase lives in this repo. You have built the entire 40-deliverable affiliate kit on the `affiliate-seeding` branch. You also wrote the hub specification at `docs/affiliate/AESDR-AFFILIATE-HUB-SPEC.md` — that document is your source of truth for what to build.

The task: **build Phase 1 of the AESDR Affiliate Hub** at `aesdr.com/affiliates` per the hub spec. The hub is the surface a affiliate-prospect navigates to when they ask, *"can I see your website?"* It must answer that question without them needing to ask follow-ups.
...
3. The seven substantial assets live at `tools/standalone-html/` and route via `app/tools/[slug]/page.tsx`.
...
- **§1 foundational doctrines** — workshop-first, founder-backstage, a handful of affiliates (not a marketplace), borrowed-trust as merciless mirror, operator over guru, honesty as differentiator.
...
- **§3.3 voice ratios** — the affiliate hub runs 80/20 to 95/5 Rowan/Michael depending on page. Spec carries the per-page table.
...
- Treating markdown as a affiliate-facing shipping format. Markdown is internal authoring; PDFs and rendered web pages are affiliate-facing.
...
1. `docs/affiliate/AESDR-AFFILIATE-HUB-SPEC.md` — this is your build spec; you wrote it; re-read it before starting.
...
7. `tools/standalone-html/` — the seven substantial assets + the 72-Hour Strike Plan bonus. Each linked from `aesdr.com/tools/[slug]`.
...
- 'https://www.shopify.com/affiliates?term=affiliate%20programme&adid=565745246064&campaignid=15436644217&utm_medium=cpc&utm_source=google&matchtype=e&network=g&gad_source=1&gad_campaignid=15436644217&gbraid=0AAAAADp4t0qGiaTcOxYPJcWd5hvm4aGU9&gclid=Cj0KCQjwh-HPBhCIARIsAC0p3cdh2sgCi_NocdGQDlygdGL006h1ittBsWLdPVmrQF7wl0rQynuYJIIaAtgGEALw_wcB` — Shopify's affiliate affiliate resources page; the closest existing precedent for the kind of hub AESDR is building, but in a different brand register. **Do not copy structure; reference for understanding the genre, then ignore. AESDR's hub is its own thing.**

---

## [4] DETAILED TASK DESCRIPTION & RULES

**Build Phase 1 of the AESDR Affiliate Hub per the spec at `docs/affiliate/AESDR-AFFILIATE-HUB-SPEC.md`.** 7-10 pages, one application form, one PDF rendering pipeline, one new Supabase table.

### Pages to build (Phase 1)

1. `app/affiliates/page.tsx` — `/affiliates` hub home. Per spec §"Page 1.1."
2. `app/affiliates/program/page.tsx` — `/affiliates/program`. Per spec §"Page 1.2."
3. `app/affiliates/curriculum/page.tsx` — `/affiliates/curriculum`. Per spec §"Page 1.3."
4. `app/affiliates/kit/page.tsx` — `/affiliates/kit`. Per spec §"Page 1.4."
5. `app/affiliates/faq/page.tsx` — `/affiliates/faq`. Per spec §"Page 1.5."
6. `app/affiliates/apply/page.tsx` — `/affiliates/apply`. Per spec §"Page 1.6."
7. a fun game. something interactive they can just start playing and scoring points (random)
8..... what am i not thinking of?

### Components to build

Per spec §"Component library." Reusable React components in `components/affiliates/`:
...
- `app/api/affiliates/apply/route.ts` — POST endpoint. Validates form input; sends email to admissions inbox; persists to `partner_applications` Supabase table.
...
Build a pipeline that converts the kit markdown files (D19, D20, D21, D28, D31, L&D brief, kit-template/00, kit-template/10d, kit-template/13) into branded PDFs at canon §8.5 anatomy. Recommend: Puppeteer + custom HTML/CSS template that uses AESDR tokens. PDFs land at `public/affiliate-kit/[filename].pdf` and are linked from `/affiliates/kit`. Only as needed for documents that are currently in markdown format that will actually be affiliate-facing.
...
9. **No new top-level routes outside `/affiliates/*`.** The hub lives entirely under `/affiliates`. Per-affiliate pages and replay pages are Phase 2 work.
 ...
 * Spec: AESDR-AFFILIATE-HUB-SPEC.md §"Page 1.1" item 4
...
- `tools/rendered/06-d26-affiliate-promo-page.html` — affiliate-promo page with editorial split hero, 4-of-12 catalog, FAQ-lite, application form pattern. **This is the closest existing precedent to the hub's `/affiliates` page.** Lift the structural pattern; rewrite the copy for the affiliate audience.
...
- Any SaaS-affiliate-program landing page with hero copy like *"Earn passive income!"* / *"Top 1% earners get exclusive perks!"* / *"Join 10,000+ affiliates crushing their commissions!"*
...
- **Batches 1–8** (April 29 – May 2, 2026): shipped all 40 D-numbered affiliate deliverables plus 9 sub-deliverables (8 kit-template files + L&D-approver brief).
...
You wrote the hub spec at `docs/affiliate/AESDR-AFFILIATE-HUB-SPEC.md` in this conversation. Re-read it before starting; do not rewrite it.
...
- Did not check production-app code before claiming "operationally pending" on items already in production (the seven substantial assets, the 2 reveal-mechanic artifacts).
...
**Build Phase 1 of the AESDR Affiliate Hub** per the spec.

Deliverables expected from you:

1. Six Next.js pages at `app/affiliates/*/page.tsx` per spec §"Phase 1."
2. Reusable React components at `components/affiliates/` per spec §"Component library."
3. One API route at `app/api/affiliates/apply/route.ts`.
4. One Supabase migration creating the `partner_applications` table.
5. PDF rendering pipeline producing branded PDFs from kit markdown files into `public/affiliate-kit/`.
...
- Components library (`components/affiliates/`).
- Supabase migration.
- API route.
- PDF rendering pipeline.
- Sitemap update.
...
**Branch:** Continue on `affiliate-seeding` (where the kit lives). When Phase 1 is complete and reviewed, the hub-specific paths (`app/affiliates/*`, `components/affiliates/*`, `app/api/affiliates/*`, `supabase/migrations/*affiliate*`, `public/affiliate-kit/*`) get cherry-picked to `main` for production deploy. Do NOT bulk-merge `affiliate-seeding` into `main`.
...
2. **Where does the founder-backstage doctrine break the typical course-creator hub pattern?** Most affiliate hubs lead with founder credibility ("founded by [name], former VP at [company]"). AESDR doesn't. What's the equivalent credibility surface that respects canon §12.1? Answer: the canon itself, the curriculum (real production lessons), the seven substantial assets (real production HTML), and the application form (real first-call commitment). The brand-of-record carries weight; the founder doesn't appear.
...
4. **What's the difference between this hub and a Profit Pillars / Afford Anything-style hub?** The genre conventions are: hero, value prop, kit downloads, FAQ, application form. AESDR's hub uses those conventions but inverts several of them — the disqualification panel is at hero-equal hierarchy, the kit downloads come before the application form (most hubs gate the kit), the canon-doctrine pages are first-class navigation items (most hubs hide the operating doctrine). Why? Because canon §1.4 says borrowed trust is a merciless mirror — you cannot hide the operating model from an affiliate who's evaluating you, so you publish it.

5. **What can break affiliate relationships if you're careless?**
   - Hub copy that reads as marketing-affiliate-program register (canon §5 violation; affiliate notices in 30 seconds).
   ...
   - Hub copy that reads as sanitized-corporate (canon §1.5 violation; affiliate reads as inauthentic).
