# AESDR Affiliate Hub — Phased Build Specification

**Document type:** Build specification, not a deliverable. Source of truth for all affiliate-hub work; never crosses the affiliate boundary.
**Routing:** Hub lives at `aesdr.com/affiliates`.
...
**Pricing doctrine:** **Never discount.** List price applies always. Affiliates earn commission off list, buyers never see promo codes from AESDR.
**Last updated:** 2026-05-05 — amendment: codify "no internal references in affiliate-facing body copy" (founder review 2026-05-04). Prior: 2026-05-02 (Phase 0 ratified).
...
- **Lockup placement:** AESDR wordmark top-left of every hub page (`aesdr.com/affiliates` ↔ buyer-side `aesdr.com/`). No co-brand lockup on the hub itself — the hub is AESDR-side; co-brand lockups appear only on per-affiliate pages at `aesdr.com/[partner_slug]/workshop`.
...
|---|---|---|
| `/affiliates` (hub home) | 80 : 20 | Verdict-shaped headlines; one Michael margin annotation max |
| `/affiliates/program` | 90 : 10 | Operating-doctrine register; Michael appears once at close |
| `/affiliates/curriculum` | 80 : 20 | Per-lesson-card Caveat annotations are Michael (12 instances total per D31, but only 4 surfaced in hub teaser) |
| `/affiliates/kit` | 95 : 5 | Reference-document register; near-zero voice |
| `/affiliates/faq` | 80 : 20 | Verdict mode; Q07-style honest disqualification carries Michael |
| `/affiliates/apply` | 90 : 10 | Operator-to-operator; one Michael line at footer |

### No internal references in affiliate-facing body copy

Per founder review 2026-05-04: affiliate-facing body copy must read like an operator wrote it for another operator, not like an internal authoring document leaked. Internal references are banned from rendered hub body copy. They remain valid (and required) in spec, build comments, and authoring source.

**Banned in rendered affiliate-facing body copy:**
...
- **Deliverable IDs** — `D22`, `D27`, `D31`, `the D-09 deck`, etc. Refer to the document by what a reader would call it: "the affiliateship agreement," "the affiliate-facing FAQ," "the curriculum map."
...
- **Internal route paths used as process nouns** — `apply at /affiliates/apply`, `see /affiliates/curriculum`, `the path-as-process /affiliates/...`. Link to the path; do not narrate the path. (Hrefs are fine; rendered copy "go to slash-affiliates-slash-curriculum" is not.)
- **Internal filename conventions** — kit downloads expose affiliate-facing names (`positioning-brief.pdf`, `newsletter-launch.md`), not authoring slugs (`D21-positioning-brief.pdf`, `09a-newsletter-launch.md`).
...
- **Visible placeholder syntax** — `[HOST_FIRST_NAME]`, `[HOST_LAST_NAME]`, `[PARTNER_NAME]`, `[PILOT_DISCOUNT]`, etc. should never reach a affiliate-prospect's eyes as literal bracket text. In hub copy, use prose ("the host," "we'll introduce you before your workshop date"). On per-affiliate surfaces, resolve all placeholders before publish.
...
- Code comments (top-of-file source citations: `Spec: AESDR-AFFILIATE-HUB-SPEC.md §X.Y` is a code comment, not body copy).
- Git commit messages.
- `SESSION_STATE.md`.
...
- **FTC §10.1:** Hub itself is AESDR-direct surface; no affiliate-attribution disclosure required on the hub. Per-affiliate pages at `/[partner_slug]/workshop` carry full disclosure.
...
| Counsel review of D22 v1 | Before affiliate agreement is sent | Counsel |
...
- **Phase 1 launch:** Hub indexed as a normal AESDR brand surface. `robots.txt` does not block. The hub's job is to be findable when a affiliate-prospect searches for AESDR.
- **Per-affiliate pages at `/[partner_slug]/workshop`:** `noindex, nofollow` per D26 spec — pilot pricing windows are time-bounded and shouldn't compete with the hub for organic ranking.
- **Sitemap.xml:** Add hub routes to existing sitemap; per-affiliate routes excluded.
...
- **Hub home:** `/affiliates`
- **Subpages:** `/affiliates/program`, `/affiliates/curriculum`, `/affiliates/kit`, `/affiliates/faq`, `/affiliates/apply` (Phase 1)
- **Phase 2 additions:** `/affiliates/preview`, `/affiliates/workshop-format`, `/affiliates/case-studies`, `/affiliates/disclosure`, `/affiliates/program/commission`
- **Per-affiliate pages:** `/[partner_slug]/workshop` (D26 production), `/[partner_slug]/replay/[token]` (D24 production)

---

## Phase 1 — Minimum Credible Hub

**Goal:** A relationship-manager candidate navigates to `aesdr.com/affiliates`, browses 6 pages, downloads PDFs, books an application call. They don't need to ask "where's your X?" because the hub answers it without them asking.
...
2. **Mono eyebrow** centered: `AESDR · AFFILIATES · EST. 2026`.
3. **Editorial split hero** (canon §6.3 pattern from `variants/variant-a-editorial-split.html`):
   ...
   *Note 2026-05-05:* The earlier *"Less affiliate empire. More founding vineyard."* hero was **retired in canon v1.3** the same day (vineyard / pruning / honest-yield metaphor cluster removed wholesale per founder direction — see `docs/canon-revisions/2026-05-05-tagline-retirement-v1.3-proposal.md`). The shipped headline above is the live version. The "three to five affiliates" cap was a fabrication and is also retired.
   - Cream right (60%): `--cond` 13pt label `WHAT THIS PAGE DOES`. Body in `--display` italic 700, 36pt: *"This is the page you point a affiliate-prospect at when they ask 'can I see your website?' — the operator-to-operator version."* Below: `--serif` 17pt context: *"AESDR runs pilot partnerships with community operators, bootcamp coaches, alumni networks, and creators. Workshop-first, time-boxed, non-exclusive."* Single iris CTA: `Request an affiliate conversation →` (anchors to `/affiliates/apply`).
4. **Three-pillar block** (cream, three white panels with `--light` border):
   - Pillar 1 — *Workshop-first.* Body: *"Every pilot leads with one live 60-minute workshop into the affiliate's audience, run by AESDR. The workshop earns the sale. The link merely attributes it."* Mono trail: `lift: AFFILIATE_BRAND_CANON.md §1.1`
...
10. **Footer.** Postal address, copyright, links to `/privacy`, `/terms`, `/refund-policy`. No affiliate-relationship disclosure (this is AESDR-direct surface).
...
3. Iris reservation: 2 instances of the same primary CTA, both anchored to `/affiliates/apply`. Within canon §6.4.
...
### Page 1.2 — `/affiliates/program`

**Source:** D40 README §2 (what affiliate can do without asking), §3 (what needs an approval round), §4 (what's not in the kit and why) + D22 §3 (Affiliate deliverables), §4 (AESDR deliverables), §5 (Compensation, sanitized for never-discount).

**Layout:**

1. Standard hub header.
2. Mono eyebrow: `AESDR · AFFILIATE PROGRAM · STRUCTURE`.
3. Hero (single-column, no editorial split for sub-pages): `--display` italic 700, 48pt: *"What the affiliateship actually is."*
...
7. **The artifacts strip** (reused from `/affiliates`). Mono eyebrow: `THE SUBSTANTIAL ASSETS THAT SHIP WITH ENROLLMENT`.
...
### Page 1.4 — `/affiliates/kit`

**Source:** D40 master affiliate-kit folder README §1 (folder tree) + every kit-template file already shipped at `docs/affiliate/kit-template/`.

**Layout:**

1. Standard hub header.
2. Mono eyebrow: `AESDR · AFFILIATE KIT · DOWNLOADS`.
3. Hero: `--display` italic 700, 48pt: *"The kit, in advance."*
4. Sub: *"Most affiliate programs gate the kit behind an application. We don't. The kit is the merciless mirror — if it doesn't survive your read, the affiliateship wouldn't either."* (lift canon §1.4)
5. **The kit index** — table format, three columns:
   ...
   - Download (`PDF` button or `Markdown` button — note: PDF is the standard, markdown only as fallback for copy templates affiliates need to edit)

   **Reference documents:**
   - Canon Excerpt for Affiliates — 6 sections of `AFFILIATE_BRAND_CANON.md` v1.1 — PDF
   ...
   **Promotional copy (affiliate-customizable):**
   ...
   **Co-brand assets:**
   - AESDR × Affiliate Lockup — Horizontal SVG
   - AESDR × Affiliate Lockup — Stacked SVG
   - Lockup Usage Guide (`10d`) — PDF

   **Operating cadence:**
   - 30-Day Pilot Operating Cadence (`13`) — PDF

6. **Honesty footer:** *"Three files you'll receive after we sign the affiliateship agreement — not before: per-affiliate UTM-tagged tracking links; per-affiliate co-brand lockup compositions; the affiliateship agreement itself (counsel-reviewed). Reasoning: those three are operationally specific to your pilot, not pre-clearable."*
7. CTA strip: iris button `Request the affiliateship agreement →`.
8. Footer.

**PDF rendering pipeline (Phase 1 dep):** Each kit file in `docs/affiliate/kit-template/` plus D19, D20, D21, D28, D31, L&D brief renders to PDF using a build-time pipeline. Recommend: Pandoc + a custom AESDR LaTeX template, or Puppeteer + a CSS-styled HTML template. Either approach must hit canon §8.5 PDF anatomy.

**Five-question check:** Pass. The kit page's table format is the operator-to-operator register at its purest.

### Page 1.5 — `/affiliates/faq`

**Source:** D23 affiliate-facing FAQ (Q05/Q07/Q11 lifts) + new affiliate-specific Qs derived from the canon.

**Layout:**

1. Standard hub header.
2. Mono eyebrow: `AESDR · AFFILIATES · FAQ`.
3. Hero: `--display` italic 700, 48pt: *"Twelve questions. Plain answers."*
4. Sub: *"This is the FAQ for affiliates. The buyer FAQ lives at `/syllabus`."*
5. **Twelve Q&A pairs** in expand-collapse format (default-collapsed on mobile, default-expanded on desktop). Each Q is `--display` italic 22pt; each A is `--serif` 16pt.
   ...
   **Recommended 12 Qs:**
   1. *"Why so few affiliates?"* — Lift canon §1.3.
   ...
   8. *"What does the workshop look like?"* — Brief description of D09 + link to `/affiliates/workshop-format` (Phase 2 stub).
   9. *"What about the founder?"* — Lift canon §12.1: AESDR is founder-backstage, host-fronted. Founder visible to affiliates; never to audience.
...
### Page 1.6 — `/affiliates/apply`

**Source:** D27 affiliate vetting scorecard §1 (affiliate identification) — sanitized for affiliate-facing register, reduced to 4 form fields.

**Layout:**

1. Standard hub header.
2. Mono eyebrow: `AESDR · AFFILIATES · APPLY`.
...
**Form posting:**
- Posts to a new API route `app/api/affiliates/apply/route.ts` that:
...
| `EditorialSplitHero` | `/affiliates` only | Crimson 40% / cream 60%, ghost numeral, configurable eyebrow + headline + body |
| `MonoEyebrow` | All hub pages | `--mono` 11pt 700 .25em uppercase `--muted` |
| `ThreePillarBlock` | `/affiliates` | Three white-panel-on-cream cards with mono trail line at bottom of each |
| `CatalogTeaserGrid` | `/affiliates` (4-card), `/affiliates/curriculum` (12-card) | Configurable card count; each card carries call number, title, question, annotation, stamp |
| `ToolStrip` | `/affiliates`, `/affiliates/curriculum` | Five named tool cards in horizontal row; links to `/tools/[slug]` |
| `DisqualificationPanel` | `/affiliates`, `/affiliates/curriculum` (variant) | White-panel-on-cream, `--cond` eyebrow, `--serif` italic body bullets |
| `CaveatLayer` | `/affiliates`, `/affiliates/apply` | `--hand` 22pt `--crimson` rotation `-1deg`, configurable copy |
| `HubCTA` | All hub pages | Single iris button, configurable text and href |
| `KitDownloadTable` | `/affiliates/kit` | Three-column table with download buttons |
| `FAQAccordion` | `/affiliates/faq` | Expand-collapse Q&A pairs |
| `ApplicationForm` | `/affiliates/apply` | 5-field form with inline validation |
...
|---|---|---|---|
| `admissions@aesdr.com` mailbox + DNS | Pending Phase 0 #6 | Founder + ops | `/affiliates/apply` form submission |
| PDF rendering pipeline (markdown → branded PDF) | Not started | Engineering | `/affiliates/kit` downloads |
| Supabase `partner_applications` table migration | Not started | Engineering | `/affiliates/apply` form persistence |
| Counsel review of D22 v1 | Operationally pending | External | `/affiliates/program` "request the agreement" CTA earns its honesty |
| Discount-doctrine cleanup across kit content | Not started | Authoring | `/affiliates/program` pricing strip; D28 v2 PDF |
| Tools page (`/tools/[slug]`) routing verification | Live, needs hub-link audit | Engineering | `/affiliates` tool strip + `/affiliates/curriculum` |
...
7. **"What this is and isn't" block:** *"This is one of twelve lessons. The other eleven cover the catalog at `/affiliates/curriculum`. The full lessons run longer and include worksheets, the substantial assets, and the Discord (Untamed). The preview is the preview."*
...
### Page 2.2 — `/affiliates/workshop-format`
...
### Page 2.3 — `/affiliates/case-studies`
...
4. Sub: *"AESDR pilots run for 30 days each. We document each one in writing. The case studies below are the affiliate-approved versions — internal versions stay internal. Per canon §13."*
5. **Case study cards** — vertical list, each with: pilot ID, affiliate archetype, headline outcome, link to full case study PDF.
...
### Page 2.4 — `/affiliates/disclosure`
...
**Source:** D26 affiliate-promo page spec + 06-rendered HTML.

**Production work:** rebuild as a Next.js dynamic route at `app/[partner_slug]/workshop/page.tsx`. Server-rendered. Pulls affiliate config from a new Supabase table `partner_pilots` (one row per affiliate, with cohort_id, partner_id, partner_type, pilot dates, partner_quote-or-null). Hidden form fields per D26 §8.

**Activates per pilot:** one route per active pilot. After `[PILOT_END_DATE]`, route 301-redirects to `aesdr.com/enroll`.

### Per-affiliate pages — `/[partner_slug]/replay/[token]`
...
### Page 3.1 — `/affiliates/how-we-work`

**Source:** Canon §1, §3, §10, §12, §13 (sanitized for affiliate-facing).
...
### Page 3.2 — `/affiliates/who-we-don't-work-with`

**Source:** D27 vetting scorecard §"Red-flag list" (sanitized).

**Layout:** Honest-disqualification at scale. Ten bullets of affiliate archetypes AESDR specifically declines, each with a one-sentence reason. Uncomfortable on purpose. Per canon §1.6 + §13.

### Page 3.3 — `/affiliates/calendar`
...
Activates when ≥3 pilots have completed. Until then, `/affiliates/case-studies` carries the function.
...
| File | Change | Effort |
|---|---|---|
| `D28-pricing-and-promo-sheet.md` | Full rewrite. Remove §2 (pilot pricing table), `[PILOT_DISCOUNT]` placeholder, `[PILOT_CODE]` references. Replace with: *"Pricing is list. Always. Affiliates earn 30% commission off list price. AESDR does not run promotional codes, pilot discounts, affiliate-stack discounts, or time-limited price drops. Buyers see the same price every other AESDR buyer sees."* Plus add the $40 follow-on as a real revenue-line section. | 1 hr |
...
| `D26-affiliate-promo-page.md` §3b CTA | Same. | 15 min |
| `D40-master-affiliate-kit-readme.md` §2.4 (pricing) | Update to reflect never-discount. | 15 min |
...
| Counsel review of D22 v1 | Legal | Counsel | Before any affiliate D22 send (Phase 1 doesn't ship D22 publicly) |
| Per-affiliate page (`/[partner_slug]/workshop`) first publish | Co-brand approval | Founder + affiliate | Per pilot |
| Lesson preview clip first publish | Founder review of cut | Founder | Before Phase 2.1 publish |
| First case study external publish | Affiliate sign-off | Founder + affiliate | Per case study |
...
5. **Hub footer.** Should the hub footer carry a separate "affiliate-relations contact" line? Recommend: no. The application form is the contact path; founder backstage doctrine (canon §12.1) means no founder email is published.
...
- Not affiliate-facing. This document never crosses the affiliate boundary. It is internal AESDR + Claude authoring context.
- Not a marketing document. The hub itself is affiliate-facing; this document is build infrastructure.
...
| Substantial assets | `tools/standalone-html/` + routes at `app/tools/[slug]/page.tsx` |
...
| 40 affiliate deliverables index | `docs/affiliate/00-INDEX.md` |
