...
> **Placeholder convention:** `[FIRST_NAME]` for recipient. `[CODE_EXPIRY_DATE_TIME_TZ]` for the actual deadline timestamp (e.g., `2026-05-17 23:59 ET`). `[HOST_FIRST_NAME]` for the host (referenced in PS only). `[PILOT_CODE]` for the pilot promo code. `[PARTNER_SLUG]` for the affiliate attribution slug. Swap globally before send.
...
`AESDR` wordmark, `--display` italic 900wt, `--ink`, on `--cream`. Co-branded `AESDR × [AFFILIATE]` lockup top-center per canon §6.6 when applicable.
...
>
> ***Will it run again?*** *Yes — at list price. The pilot pricing is specific to [PARTNER_SLUG] and to this pilot window. We're running other pilots with other affiliates; pricing for those is theirs, not ours to extend here.*
>
...
Workshop hosted by AESDR in partnership with [affiliate]. [Affiliate] receives
...
- **Hide the real reason for the deadline.** Our reason is honest: pilot pricing is affiliate-specific and time-boxed; replays expire to keep the workshop live, not evergreen. We say so.
...
- **A real, single deadline** — the pilot pricing window must actually close at the timestamp. Operationally: per canon §9.2, this must be enforced in Stripe (code expires) and on the affiliate-promo page (offer surface reverts to list). **Operationally pending per pilot setup.**
...
- **D34 affiliate-facing close-out** — references the pilot's deadline-day conversion as part of the recap.
