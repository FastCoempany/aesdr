# D40 — Master Affiliate-Kit Folder README

**Deliverable:** The single-page index that opens the affiliate-kit folder. The first document an affiliate reads when they receive the kit. Tells them what's in the folder, what each file is for, what they may edit, what they may not, and where to put their own inputs. This is the deliverable that fixes the "twelve mystery crumbs in Slack" problem named in the affiliate-seeding research report's enablement-kit table.
**Audience:** Affiliate principal + anyone on Affiliate's team writing or producing AESDR-promoting copy. Sent within 3 business days of D22 signing per D22 §4.5.
**Voice ratio:** 90 Rowan / 10 Michael per canon §3.3 (internal docs row, affiliate-adjacent variant). Operator-to-operator. The Michael line is the closer.
...
> *Hi [PARTNER_NAME] —*
>
> *This folder is the AESDR pilot kit for the affiliateship we signed on [PILOT_START_DATE]. It's intentionally small. We work with a handful of affiliates, not a marketplace, and every file in here earns its place.*
>
...
├── 10-co-brand-lockup/                          ← AESDR × [AFFILIATE] assets
...
├── 12-objection-replies.pdf                     ← D05 (for Affiliate team Q&A)
└── 13-operating-cadence.md                      ← what happens in what week
```

Files numbered `00`–`08` are reference. Files `09`–`13` are operational — the ones Affiliate uses while running the pilot.
...
### 2.2 Use the AESDR × Affiliate co-brand lockup as supplied

Files `10a`–`10c` are the lockup in three render targets. Use them on:
- Your registration / promo emails.
- Co-branded landing surfaces AESDR ships (D26 affiliate-promo page, when produced).
...
Per canon §14 — the taglines listed in `00-canon-excerpt.md` are pre-cleared for affiliate use. Use, don't paraphrase.

### 2.4 Use the tracking links from `11-tracking-links.md`

Every URL in that file is UTM-tagged and affiliate-attributed. Use them exclusively — per D22 §3.4, modified or shortened links break attribution and may be treated as a §11.1 violation.

### 2.5 Send the affiliate-supplied workshop intro script

Per D09 slide 02, your 2-minute live intro is affiliate ad-libbed from the AESDR-supplied script. The script is in `09c-podcast-intro-script.md` and the live-intro variant is in `13-operating-cadence.md` for the workshop-week section.
...
| Add a third logo or co-branding element beyond the AESDR × Affiliate lockup | Pre-approval; default answer is no, per canon §6.6. |
...
- **The AESDR email list.** Affiliate does not receive it. AESDR does not request Affiliate's list. Per D22 §6.5.
...
- **Performance guarantees.** AESDR does not guarantee Affiliate a specific revenue, conversion rate, or attendance number. Per D22 §14.1. Reciprocally, Affiliate doesn't guarantee AESDR audience size.
...
| Week | What happens | What Affiliate does | What AESDR does |
|---|---|---|---|
...
| Pilot close (`[PILOT_END_DATE]`) | Final report + close-out | Confirms metrics shared. | Sends D25 Report 3 + D34 affiliate-facing close-out + initiates D32 cut-or-keep. |
...
### *"Can I share this kit with another affiliate I'm thinking of co-pilotinging with?"*
...
- **Disguise the founder–affiliate relationship.** Per canon §1.4 borrowed-trust doctrine, the affiliate's audience is being lent, not transferred. We say so.
...
Per D22 §11. In short: termination requires written notice. Affiliate removes AESDR-promoting content from owned channels within 5 business days. Commission accrued before termination remains payable. Both Parties get the close-out conversation D34 covers.
...
**Layout pattern:** The README renders as a markdown document inside the kit's host system (Notion / Google Doc / Dropbox / Drive). It is not a designed PDF — affiliates need to scan it on phone, search it in a browser, copy snippets out of it. Per canon §6.5, the discipline here is *legibility over decoration*.
...
- Where AESDR does not control render (Affiliate's GitHub view, etc.), the README still parses cleanly.

**Type tokens:** Per palette above. No Caveat — this is a affiliate-side ops document, not an audience-facing voice surface.
...
**Deliberate departures from canon:** The README's hostility to a designed PDF render *is* a canon decision — affiliates need to use this document as a workspace tool, not admire it. Per canon §6.9.1 thumbnail test, the README at thumbnail size is identifiable by section structure and Welcome paragraph register, not by visual fingerprint.

**Five-question check (canon §6.9.1):**
1. **Thumbnail at 200×200:** Even rendered as plain markdown, the *Welcome* paragraph and the verdict-shaped *"intentionally small. We work with a handful of affiliates, not a marketplace"* line are identifiable. Pass.
...
5. **Voice thumbnail:** *"This folder is the AESDR pilot kit for the affiliateship we signed on [PILOT_START_DATE]. It's intentionally small. We work with a handful of affiliates, not a marketplace, and every file in here earns its place."* — passes. Identifiably AESDR; verdict-shaped Rowan, plain language, no metaphor (per canon §1.3 v1.3).
...
- **Every D-file referenced in §1's folder-tree.** The kit is the assembly. Most exist (D19, D20, D21, D22, D28, D31). Pending in this batch's downstream: D29 host bio (file `07`), D30 lesson preview (file `08`), and D26 affiliate-promo page (referenced).
- **Co-brand lockup assets** (`10a`–`10d`). **Operationally pending** — produced once an affiliate is PASSed; `10d-lockup-usage-guide.md` is canonical (write once, lift per affiliate with `[PARTNER_NAME]` swap).
- **Tracking links file** (`11-tracking-links.md`). **Operationally pending per pilot** — UTMs and codes are affiliate-specific.
- **Promotion copy pack** (`09a`–`09d`). **Operationally pending** — generated from D02, D03, D04 register and affiliate-customized.
...
- **D33 postmortem** — references "did the affiliate read the README" as a qualitative-signal input.

---

## Open

- **Host system.** Default: Notion (mirror to Google Drive for affiliates who prefer it). Alternative: a static-site-generator render at `kit.aesdr.com/[PARTNER_SLUG]` per pilot, with cleaner permission handling. Recommend: **Notion for v1**, sg-rendered later if pilot count crosses 5.
- **Whether to ship a `00-canon-excerpt.md` instead of the full canon.** Default: **excerpt** — affiliates need the 6 sections relevant to them, not the 17. Full canon stays internal. Risk: excerpt drifts from canon. Mitigation: the excerpt is generated from the canon source, not authored separately.
- **Affiliate team-member access.** Default: kit is shareable read-only with anyone Affiliate authorizes; sensitive items (D22 signed PDF, the unredacted disclosure-pack with internal commentary) are restricted. Recommend codifying the team-member access policy as `13-operating-cadence.md` §X rather than here.
...
- **Whether the §6 common-questions list should be lifted to a affiliate-facing FAQ surface.** Default: **stays in this README.** Affiliates read the README; prospects read the public FAQ (D23). Different audiences, different surfaces. Don't blur them.
