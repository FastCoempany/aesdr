...
When co-branded: `AESDR × [affiliate]` lockup top-center per canon §6.6.
...
The ICS attachment includes a `LINK` field to a calendar invite cover image at `aesdr.com/calendar-cover/[affiliate-slug]/[workshop-slug].png`.

**Cover spec (1280×720):**
- Background `--cream`.
- AESDR wordmark `--display` italic 900wt, top-left, ~64pt.
- Affiliate co-mark top-right (when co-branded).
