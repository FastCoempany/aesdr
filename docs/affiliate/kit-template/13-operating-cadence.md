...
>
> **Reading the table:** Each row is a calendar day or a date-range. The "Affiliate" column lists what you do; the "AESDR" column lists what AESDR does. "Joint" rows are when both parties act together.
...
| T-19 | Kick-off call (30 min). Confirm send schedule, audience descriptor, affiliate intro reason. Surface any kit-content questions. | Founder runs kick-off call. Capture affiliate intro reason for `09c` script. |
...
| T-17 | Confirm `[YOUR_FIRST_NAME]`, `[YOUR_AUDIENCE_DESCRIPTOR]`, and any optional affiliate-quote per D26 §6. | Issue kit-PDF render of your affiliate-quote slot for D26. Surface any approval edits. |
...
**By end of T-15:** Kit confirmed. Send `09a` ready to publish. Tracking working. Co-brand assets approved. Affiliate intro script (`09c` Script 1) practiced once.
...
| T-3 | **Tech rehearsal joint call** — affiliate intro practiced (2 min), workshop deck slides 01–03 walked, AV checked end-to-end. 45 min. | Run tech rehearsal. Capture any final edits to affiliate intro script. |
...
**End of week 2 signal:** All registrants have received the 24h reminder. Tech rehearsal clean. Affiliate intro practiced.

---

## Workshop day (T-0)

| Time | Affiliate | AESDR |
|---|---|---|
| Pre-event | Final review of affiliate intro. Show up 10 min early. | All systems live. Founder lurks under alias per canon §12.2. |
...
| Day | Affiliate | AESDR |
|---|---|---|
| T+8 | (Light week — no new affiliate action required.) | Continue D17 follow-up replies. Begin internal D32 cut-or-keep memo draft (canon §16). |
| T+10 | (No affiliate action.) | Founder + ops complete D32 memo. Decision: EXTEND / REVISE / CUT. |
| T+12 | Receive D34 affiliate-facing close-out note (extend or cut version). 48 hrs after D32 finalized. | Send D34. |
...
| T+45 | Receive net-45 commission payout per D22 §5.4. | Pay commission to affiliate. |
...
- **Affiliate missed launch send by 24h:** Email `[OPS_EMAIL]`. AESDR shifts the cadence by 24h or — if the workshop date is fixed — runs a compressed promotion with a single reminder. Not a §11 issue; one slip is data, not a violation.
- **Affiliate missed both launch and reminder sends:** §16 conversation. Two missed sends without notice is a §11.1 issue.
...
- **Affiliate publishes a non-cleared social post:** AESDR asks for the edit per D40 §3. If affiliate refuses, §11.1.
- **Affiliate asks for a deadline extension:** Default no per canon §9.2 anti-fake-urgency. Exception: if AESDR caused a delay (tech failure, missed kit handoff), extension is communicated as a separate honest acknowledgment, not by re-sending D18.
- **Audience drama / outage / external event during the workshop window:** AESDR ops + affiliate principal call-to-decide whether to delay. Real events earn real adjustments per canon §1.6.
...
| D34 (affiliate close-out) | T+12 | AESDR founder |
| D33 postmortem | T+37 (within 7 days of pilot end) | AESDR internal — not shared with affiliate |
...
*Source: D22 affiliate pilot agreement timeline + canon §7.1 workshop format + canon §1.3 (a handful of affiliates, not a marketplace) + research report §Sample 30-day pilot timeline. Per pilot, AESDR ops adjusts dates to the actual workshop schedule.*
