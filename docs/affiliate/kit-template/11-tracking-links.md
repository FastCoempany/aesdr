...
>
> **Per canon §8.8:** Attribution runs from first qualifying click through 30 days. Refunds reduce the commission base proportionally. If a affiliate-specific code (`[PILOT_CODE]`) is applied at checkout, the affiliate is the attributed source regardless of last-click.
>
...
| Affiliate type | `[community / coach / creator / alumni / hybrid]` |
| Pilot ID | `[PILOT_ID]` (e.g., `c_2026-05-apex`) |
| Cohort ID | `[COHORT_ID]` (often same as Pilot ID) |
| Internal affiliate ID | `[PARTNER_ID]` (e.g., `p_001`) |
...
| Surface | URL |
|---|---|
| Newsletter — launch send (`09a`) | `https://aesdr.com/[PARTNER_SLUG]/workshop?utm_source=[PARTNER_SLUG]&utm_medium=affiliate&utm_campaign=[PILOT_ID]&utm_content=newsletter-launch&partner_id=[PARTNER_ID]&partner_type=[PARTNER_TYPE]&cohort_id=[COHORT_ID]` |
...
| Surface | URL |
|---|---|
| Replay link in member channel | `https://aesdr.com/[PARTNER_SLUG]/replay/[TOKEN]?utm_source=[PARTNER_SLUG]&utm_medium=affiliate&utm_campaign=[PILOT_ID]&utm_content=replay-direct&partner_id=[PARTNER_ID]&partner_type=[PARTNER_TYPE]&cohort_id=[COHORT_ID]` |
...
| Surface | URL |
|---|---|
| Direct enrollment | `https://aesdr.com/enroll?utm_source=[PARTNER_SLUG]&utm_medium=affiliate&utm_campaign=[PILOT_ID]&utm_content=direct-enroll&partner_id=[PARTNER_ID]&partner_type=[PARTNER_TYPE]&cohort_id=[COHORT_ID]&code=[PILOT_CODE]` |
...
- **Last-click.** Per canon §8.8, the affiliate-code application overrides last-click. If a buyer arrives via your link, drifts away, comes back via Google, and applies `[PILOT_CODE]` at checkout, you're still the attributed affiliate.
- **Browser cookies alone.** Server-side attribution captures the affiliate relationship at first qualifying click; cookies are belt-and-suspenders, not the system of record.
...
The previous version of this file is archived at `docs/affiliate/pilots/[PARTNER_SLUG]/tracking-links--v[N].md`. The current version is the authoritative one.

---

*Source: canon §8.7 (UTM canon) + canon §8.8 (attribution windows). Per pilot, generated by AESDR ops; not affiliate-editable.*
