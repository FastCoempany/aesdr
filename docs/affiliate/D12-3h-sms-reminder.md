...
**Audience:** SMS-consented registrants only. The default expectation is that a workshop sends zero SMS unless a affiliate pilot has SMS in scope.
...
- Use only when affiliate-pilot audience tolerates a slightly-personal tone. Not the right default for the most formal affiliate contexts.
...
That consent does **not** authorize AESDR's *affiliates* to text the same registrant. If an affiliate wants to text its own list about AESDR, the affiliate is the seller of record on its own consent (canon §10.5).
...
- **Default for the first pilot is Version A.** Versions B and C come online once we know whether the affiliate audience tolerates branded specificity (B) or personal warmth (C) over plain.
...
- **Opt-in language at registration is canon (D7 form spec).** If the affiliate pilot does not run with SMS, this entire deliverable is dormant for that pilot. That's fine — most early pilots probably should not run SMS until we have email-side data.
