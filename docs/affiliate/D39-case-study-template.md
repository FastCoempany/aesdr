...
**Deliverable:** Template for AESDR's canonical case-study format. Used post-pilot to codify what worked, what didn't, and what the affiliateship taught AESDR — in a register that's externally shareable (with permission) and internally durable (the case study survives the affiliate). One template; multiple instances per affiliate; the most useful artifact AESDR produces for future-affiliate conversations.
**Audience:** Two intended audiences, same source document with different render permissions:
- **Internal** — AESDR-controlled artifact filed at `docs/affiliate/pilots/[PARTNER_SLUG]/case-study--v1.md`. Used during D27 vetting calls with prospective new affiliates ("here's how a similar archetype's pilot ran") and during ambassador-conversion conversations.
- **External (with explicit affiliate approval)** — a affiliate-cleared version may be shipped publicly: AESDR's `/case-studies` page (when warranted), affiliate-promo collateral, conference / podcast contexts. **Affiliate sign-off on the external version is required per canon §16; the external version is a redacted/condensed render of the internal source.**
...
**Format:** Markdown source. Renders to (1) internal markdown reference filed in pilot folder, (2) external PDF case-study one-pager (when affiliate-approved), (3) external HTML page on `aesdr.com/case-studies/[PARTNER_SLUG]` (when warranted by post-launch traffic).
...
> **Placeholder convention:** `[PARTNER_NAME]`, `[PARTNER_SLUG]`, `[PILOT_ID]`, `[PILOT_DATES]`, `[PARTNER_ARCHETYPE]`, `[FOUNDER_FIRST_NAME]`, plus pilot-specific numeric placeholders. Internal version retains full names; external version uses `[PARTNER_NAME]` only when affiliate approves naming, otherwise paraphrases archetype.

---

## Why a case study is different from a postmortem

D33 (postmortem) is internal critique. It includes "what broke," "anti-canon moves we let slip," and "lessons that survive the affiliate." It's not safe to share externally — it contains analysis the affiliate shouldn't read.

D39 (case study) is codification. It captures *what the pilot taught AESDR* in a register that's safe to share with a future-affiliate-prospect or — with the affiliate's signoff — to publish externally.

| Postmortem (D33) | Case study (D39) |
|---|---|
| Internal-only | Internal source + affiliate-approvable external |
...
| Never shared with affiliate | Affiliate-cleared version may be shared with permission |
...
> *[PILOT_DATES] · Affiliate archetype: [PARTNER_ARCHETYPE] · Filed [YYYY-MM-DD]*
...
**Internal version** is more direct. **External version** is structurally identical but reads as operator-to-operator affiliate-recommendation register.
...
> *In May 2026, AESDR partnered with the Apex BDR Club for a 30-day pilot — one live workshop, a 72-hour replay, and a affiliate-attributed enrollment window. The pilot reached 47 enrollments with 78% attendance and a 2% refund rate. AESDR is keeping the model — workshop-first, affiliate-co-branded, time-boxed, non-exclusive.*

---

### 2. Affiliate archetype + ICP fit

> *(--serif 16pt body)*

| Field | Value |
|---|---|
| Affiliate archetype | `[community / coach / creator / alumni / hybrid]` |
...
The "why this archetype fits AESDR" beat is the codification. It's the answer AESDR gives a future-affiliate-prospect from the same archetype: *"Here's why pilots in your bucket land where they land — in the operating logic, not in vibes."*
...
The structural sequence of the pilot, affiliate-side and AESDR-side, with named-deliverable references for traceability.

- **Pre-pilot:** Affiliate passed D27 vetting at [score]. Pilot terms signed (D22). Kit handed off ([N_DAYS] before promotion window).
- **Promotion week 1:** Affiliate sent `09a-newsletter-launch.md` template-derived launch on [date]. AESDR built D26 affiliate-promo page. UTM trail from `11-tracking-links.md` confirmed live.
- **Promotion week 2:** Affiliate sent reminder `09b`. AESDR began D17 high-intent DM outreach to T1/T2 triggered prospects.
- **Workshop day:** Affiliate delivered 2-min intro per `09c`. AESDR host delivered D09 deck. [N] live attendees; [N] chat questions logged.
...
- **Pilot close:** D32 cut-or-keep memo: [decision]. D34 affiliate-facing close-out sent. D33 postmortem filed.
...
| Commission accrued (Affiliate) | — | [$Y] | |
| Founder hours | — | [n] | |
| Effective hourly rate | — | [$/hr] | Net rev / founder hours. |

**Internal version** carries every row. **External version** strips Founder hours + Effective hourly rate (those are AESDR-internal economics; affiliate doesn't see them in published case studies).

**Per canon §10.2 approved claims:** Numbers above are observational data from the pilot, not promises about future pilots. **Per canon §10.3:** No income claims about the affiliate's audience members; no per-seat revenue projections.
...
Each beat is canonical for the archetype. AESDR can lift "this is what landed in [archetype] pilots" into the next D27 conversation with a similar affiliate-archetype prospect.
...
The discipline: **no scapegoating.** Per canon §1.6, the affiliate is an operator; if the affiliate-side execution was hard, frame it as operating-fit signal that AESDR needs to surface earlier in vetting (D27 update), not as affiliate failure.
...
Operating changes only. Not "we wish the affiliate had more reach" — that's not actionable. *"We'd start the L&D-brief sends 48 hours earlier"* is actionable.
...
> *(--display italic 28pt for affiliate quote; --cond eyebrow attribution)*

If the affiliate explicitly approves a quote (per canon §16 + D27 conversation), it lands here. **No quote without written approval.** **No approximate quotes.** Verbatim only.

> *"[PARTNER_QUOTE_VERBATIM]"*
>
> `— [PARTNER_QUOTE_ATTRIBUTION] · [PARTNER_NAME]`

Per canon §10.3, affiliate quotes that imply income / outcomes / promises are forbidden — even if the affiliate offers them. AESDR may decline a quote the affiliate enthusiastically offers if it crosses canon §10.3.
...
### 10. Status of affiliate relationship after pilot
...
| Affiliate principal (external version only) | `[PARTNER_PRINCIPAL_NAME]` | `[YYYY-MM-DD]` |

Internal version: founder signs alone.

External version: affiliate principal sign-off is **required**. The affiliate-side approval covers (a) the quote in §8 if any, (b) any mention of affiliate-internal data, and (c) the case study's overall framing. Per canon §16, no external publication without dual sign-off.

Filed to: `docs/affiliate/pilots/[PARTNER_SLUG]/case-study--v1.md` (internal source) + `case-study--external--v1.md` (affiliate-approved external render, if any).

---

## Render-target permissions matrix

| Section | Internal | External (affiliate-approved) |
|---|---|---|
| 1. Pilot at a glance | Yes (direct register) | Yes (affiliate-recommendation register) |
| 2. Affiliate archetype + ICP fit | Yes (full) | Yes (without internal scoring rubric) |
...
| 6. What was harder | Yes | Yes (with affiliate approval — can be sensitive) |
...
| 10. Affiliate status | Yes | Partial (extension + sunset stripped) |
| 11. Sign-off | Founder only | Founder + Affiliate |
...
- Affiliate quote (§8): `--display` italic 700, 28pt, `--ink`.
- Affiliate attribution: `--cond` 11pt, .15em, uppercase, `--muted`.
...
1. **Thumbnail at 200×200:** Cream + display italic title + mono eyebrow + clean tables + affiliate quote = identifiably AESDR (codification register). Pass.
2. **Token check:** Pass.
...
- **FTC §10.1:** External version, when published, names AESDR's commercial relationship with the affiliate (the pilot's commercial structure is in the case study itself, so disclosure is implicit; explicit footer added if hosted on a public AESDR page). **Pass conditional on render.**
...
- **Forbidden claims (§10.3):** No "if you affiliate with AESDR your audience will earn $X" / "AESDR pilots produce N% conversion guaranteed" framing. The case study reports what happened in *this* pilot, not what will happen in others. **Pass.**
...
- **D34 affiliate-facing close-out** — affiliate-side relationship status (§10) reflects D34 outcome. **Met.**
- **D27 affiliate vetting scorecard** — §2 archetype data + §9 scorecard-vs-reality delta. **Met.**
...
- **External publication threshold.** Default: **affiliate-approved version only**, never auto-published. Internal source filed regardless. Ratio expected: ~1-in-3 pilots produces an external case study; the rest stay internal codification.
- **Whether to publish redacted external versions** that anonymize the affiliate. Default: **no**. An anonymized external case study reads as fabricated; the value is in named affiliate specificity. If the affiliate declines naming, the case study stays internal — that's a reasonable outcome.
- **Whether to ship a "trends across N pilots" meta-case-study**. Default: **not in v1.** Cohort-level trends live in D37 dashboard's View 3 (internal). A public-facing trends document would either reveal cross-affiliate data (canon §12 confidentiality) or be vague enough to be marketing fluff. Reconsider only after 10+ pilots.
...
- **Affiliate ability to redact** specific data points from the external version. Default: **yes, on request, within reason.** Affiliate may redact quote, redact specific numbers, redact archetype detail. AESDR may decline to publish if the redactions remove enough that the case study becomes vague. Per canon §13.
- **Whether to cross-link case studies in subsequent affiliate kits** (D40). Default: **yes, for similar-archetype pilots.** A new affiliate from the community-operator archetype gets the prior community-operator case study (or a similar one) referenced in their kit's `00-canon-excerpt.md` or in the §1 welcome of D40. Specific case-by-case based on the new affiliate's vetting conversation.
